...
?>
    ...
    <div class="Monster_search">
        <form action="searchDiscussion.php" method="get">
        <div class="input-group mb-3" style="margin-bottom: 0!important;">
            <div class="input-group-prepend">
                <button class="btn btn-outline-secondary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Discussions</button>
            ...
            <input type="text" name="term"class="form-control" placeholder="Chupacabra" aria-label="search_query" aria-describedby="button-addon2">
        ...
        $sql = "SELECT DISTINCT post_title, user_name, post_time_date, mon_name
            FROM discussion_post, monster, monster_names
            WHERE monster_names.moID = monster.ID
            AND monster.ID = discussion_post.monsterID
            AND ((mon_name LIKE '%".$term."%') 
                OR user_name LIKE '%".$term."%' 
                OR post_title LIKE '%".$term."%' 
                OR post_time_date LIKE '%".$term."%')";
        if ($term != "")
        {
