    ...
    <div class="Monster_search">
        ...
        <div class="input-group mb-3" style="margin-bottom: 0!important;">
            <div class="input-group-prepend">
                <button class="btn btn-outline-secondary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Monsters</button>
            ...
            <input type="text" name="term"class="form-control" placeholder="Mothman" aria-label="search_query" aria-describedby="button-addon2">
        ...
        $sql = "
        SELECT DISTINCT ID, mon_name, type, description
        FROM (monster LEFT OUTER JOIN traits on ID = monID), monster_names, location, lives_in
        WHERE monster_names.moID = monster.ID
        AND (mon_name LIKE '%".$term."%'
        OR description LIKE '%".$term."%'
        OR diet like '%".$term."%'
        OR type LIKE '%".$term."%'
        OR (lives_in.monst_ID = ID AND lives_in.CoStPr = CSP AND (CSP LIKE '%".$term."%'
