...
?>
    ...
    <div class="Monster_search">
        <form action="searchLocation.php" method="get">
        <div class="input-group mb-3" style="margin-bottom: 0!important;">
            <div class="input-group-prepend">
                <button class="btn btn-outline-secondary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Locations</button>
            ...
            <input type="text" name="term"class="form-control" placeholder="Loch Ness" aria-label="search_query" aria-describedby="button-addon2">
        ...
        $sql = "SELECT DISTINCT CSP, country, environment
            FROM location, (monster LEFT OUTER JOIN traits ON monID = ID), monster_names, lives_in
            WHERE monster_names.moID = ID
            AND ((CSP LIKE '%".$term."%'
            OR country LIKE '%".$term."%'
            OR environment LIKE '%".$term."%')
            OR ((mon_name LIKE '%".$term."%' OR description LIKE '%".$term."%' OR type LIKE '%".$term."%' OR diet LIKE '%".$term."%')
                AND monster_names.moID = lives_in.monst_ID 
                AND lives_in.CoStPr = location.CSP))";
        if ($term != "")
        {
